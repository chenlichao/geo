...
int main(int argc, char** argv)
{
	...
	auto dicx = map<int, accumulator_set<double, stats<tag::mean, tag::variance>>>();
	auto dicy = map<int, accumulator_set<double, stats<tag::mean, tag::variance>>>();
	auto dicr = map<int, accumulator_set<double, stats<tag::mean, tag::variance>>>();
	...
	for (auto it = dicx.begin(); it != dicx.end(); it++){
		...
		if (ind > total*progress / 100){
			cout << progress << " has been done \t" << ind <<
		...
		if (c > 1){
			fprintf(fout,"%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\n", it->first, c, 
					variance(it->second)*c / (c - 1), variance(dicy[it->first])*c / (c - 1),
					variance(dicr[it->first])*c / (c - 1),
					mean(it->second), mean(dicy[it->first]),
					mean(dicr[it->first])
					);
		}
		else {
